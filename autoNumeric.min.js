...
"\\d]\\D*$");e=(b.aNeg?b.aNeg:"-")+b.aNum+b.aDec;if(b.altDec)e+=b.altDec;b._allowed=RegExp("[^"+e+"]","gi");b._numReg=RegExp(d+"(?:\\"+b.aDec+"?(\\d+\\"+b.aDec+"\\d+)|(\\d*(?:\\"+b.aDec+"\\d*)?))");return b}function j(a,c,b){if(c.aSign)for(;a.indexOf(c.aSign)>-1;)a=a.replace(c.aSign,"");a=a.replace(c._skipFirst,"$1$2");a=a.replace(c._skipLast,"$1");a=a.replace(c._allowed,"");if(c.altDec)a=a.replace(c.altDec,c.aDec);a=(a=a.match(c._numReg))?[a[1],a[2],a[3]].join(""):"";if(b){c="^"+c._aNegReg+"0*(\\d"+
(b==="leading"?")":"|$)");c=RegExp(c);a=a.replace(c,"$1$2")}return a}function s(a,c,b){if(c&&b){var d=a.split(c);if(d[1]&&d[1].length>b)if(b>0){d[1]=d[1].substring(0,b);a=d.join(c)}else a=d[0]}return a}function n(a,c,b){if(c&&c!==".")a=a.replace(c,".");if(b&&b!=="-")a=a.replace(b,"-");a.match(/\d/)||(a+="0");return a}function t(a,c,b){if(b&&b!=="-")a=a.replace("-",b);if(c&&c!==".")a=a.replace(".",c);return a}function o(a,c){a=j(a,c);a=s(a,c.aDec,c.mDec);a=n(a,c.aDec,c.aNeg);var b=a*1;return b>=c.vMin&&
b<=c.vMax}function p(a,c){a=j(a,c);if(a===""||a==c.aNeg)return c.wEmpty=="zero"?a+"0":c.wEmpty=="sign"?a+c.aSign:a;var b="";b=c.dGroup==2?/(\d)((\d)(\d{2}?)+)$/:c.dGroup==4?/(\d)((\d{4}?)+)$/:/(\d)((\d{3}?)+)$/;var d=a.split(c.aDec);if(c.altDec&&d.length==1)d=a.split(c.altDec);var e=d[0];if(c.aSep)for(;b.test(e);)e=e.replace(b,"$1"+c.aSep+"$2");if(c.mDec!==0&&d.length>1){if(d[1].length>c.mDec)d[1]=d[1].substring(0,c.mDec);a=e+c.aDec+d[1]}else a=e;if(c.aSign){b=a.indexOf(c.aNeg)!==-1;a=a.replace(c.aNeg,
"");a=c.pSign=="p"?c.aSign+a:a+c.aSign;if(b)a=c.aNeg+a}return a}function q(a,c,b,d){a=a===""?"0":a+="";var e="",f=0,i="";if(a.charAt(0)=="-"){i=a*1===0?"":"-";a=a.replace("-","")}if(a*1>0)for(;a.substr(0,1)=="0"&&a.length>1;)a=a.substr(1);f=a.lastIndexOf(".");if(f===0){a="0"+a;f=1}if(f==-1||f==a.length-1)if(d&&c>0){e=f==-1?a+".":a;for(f=0;f<c;f++)e+="0";return i+e}else return i+a;var h=a.length-1-f;if(h==c)return i+a;if(h<c&&d){e=a;for(f=h;f<c;f++)e+="0";return i+e}d=f+c;h=a.charAt(d+1)*1;var k=[];
for(f=0;f<=d;f++)k[f]=a.charAt(f);a=a.charAt(d)=="."?a.charAt(d-1)%2:a.charAt(d)%2;if(h>4&&b==="S"||h>4&&b==="A"&&i===""||h>5&&b==="A"&&i=="-"||h>5&&b==="s"||h>5&&b==="a"&&i===""||h>4&&b==="a"&&i=="-"||h>5&&b==="B"||h==5&&b==="B"&&a==1||h>0&&b==="C"&&i===""||h>0&&b==="F"&&i=="-"||h>0&&b==="U")for(f=k.length-1;f>=0;f--)if(k[f]!="."){k[f]++;if(k[f]<10)break}for(f=0;f<=d;f++)e+=k[f]=="."||k[f]<10||f===0?k[f]:"0";if(c===0)e=e.replace(".","");return i+e}function u(a,c){this.options=c;this.that=a;this.$that=
g(a);this.formatted=false;this.io=l(this.$that,this.options);this.value=a.value}function r(a){if(typeof a=="string"){a=a.replace(/\[/g,"\\[").replace(/\]/g,"\\]");a="#"+a.replace(/(:|\.)/g,"\\$1")}return g(a)}g.extend(u.prototype,{init:function(a){this.value=this.that.value;this.io=l(this.$that,this.options);this.cmdKey=a.metaKey;this.shiftKey=a.shiftKey;var c=this.that,b={};if(c.selectionStart===undefined){c.focus();var d=document.selection.createRange();b.length=d.text.length;d.moveStart("character",
